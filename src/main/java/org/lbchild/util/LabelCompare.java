...
public class LabelCompare {
	public void compare(String user1path, String user2path) {
		...
		{
			labelToList(user1List.get(i).get("NEWSMARK"),labelList1);
			labelToList(user2List.get(i).get("NEWSMARK"),labelList2);
			List<Integer> diflist=compareLabelList(labelList1, labelList2);
			double rate=(1-diflist.size()/labelList1.size())*100;
			System.out.print("一致率为"+rate+"%");
			if(!diflist.isEmpty())
			{
